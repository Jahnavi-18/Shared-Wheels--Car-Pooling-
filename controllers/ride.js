...
export const getRide = async (req, res, next) => {
  try {
    const ride = await Ride.findById(req.params.id)
      .populate(
        "creator",
        "name age stars rating profile ridesCreated createdAt"
      )
      .lean();
    if (!ride) {
  ...
  try {
    ...
    const rides = await Ride.find({
      ...
      startTime: {
        ...
        $lt: new Date(searchDate.getTime() + 24 * 60 * 60 * 1000).toISOString(),
      }, // Filter rides up to next midnight
    })
...
export const joinRide = async (req, res, next) => {
  try {
    ...
    if (ride.passengers.includes("662a542f2ead81fcaffb6bc7")) {
    ...
    await Ride.updateOne(
      { _id: ride._id },
      {
        $push: { passengers: "662a542f2ead81fcaffb6bc7" },
        $inc: { availableSeats: -1 },
      }
    ),
      await User.updateOne(
        { _id: "662a542f2ead81fcaffb6bc7" },
...
export const createRide = async (req, res, next) => {
  try {
    const newRide = new Ride({
      ...req.body,
      creator: "662f6e23aeba91e611723983",
    });
    await newRide.save();
...
export const deleteRide = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate("662a542f2ead81fcaffb6bc7", {
